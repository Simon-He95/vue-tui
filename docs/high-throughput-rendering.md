...
`bench:baseline` 是 PR CI 的 deterministic behavior budget check，不按 GitHub runner timing
设门槛。nightly benchmark workflow 同样跑 `bench:baseline`。手动触发 benchmark workflow 且
