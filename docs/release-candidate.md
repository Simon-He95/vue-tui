...
- root entrypoint 导出 browser-safe 的 no-op `defaultTInputHostPlugin`；Node-aware 的 `createDefaultTInputHostAdapter()` / `defaultTInputHostPlugin` 继续从 `@simon_he/vue-tui/cli` 引入。
