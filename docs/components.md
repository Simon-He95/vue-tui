...
虚拟滚动或重内容场景建议使用 `deferRenderUntilVisible`、`suspendRenderWhileScrolling` / `suspended` 和 `createTerminalGraphicRenderQueue()` 控制懒渲染、取消、缓存与并发。PNG 渲染链路可用 `createPngTerminalGraphicRenderer()` 组合 Kitty / iTerm2 序列；Sixel 需要宿主提供 `toSixel` encoder，例如 libsixel binding、`img2sixel` 包装或自定义 renderer。
