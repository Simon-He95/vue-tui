...
> Experimental API：当前从 `@simon_he/vue-tui/experimental` 导出，暂不进入 root 入口。`ansi=true` 只支持 ANSI SGR styling；OSC8 hyperlink、高亮和 arbitrary variable-height rich rows 仍不是当前能力。
...
当前只支持 ANSI SGR styling。不支持 OSC8 hyperlink、cursor movement、clear screen、alternate buffer、syntax highlight、search highlight、markdown/rich text 或 arbitrary variable-height row model。
