...
Advanced 表单上下文组件，从 `@simon_he/vue-tui/vue` 引入。它提供 validation/error context：`model`、同步 `rules`、`submit`、`validation`，以及 `TFormField name` 对错误文本的读取。`readOnly` 目前只是通过 form context 提供给自定义字段消费者的提示；后代输入控件不会自动继承 `disabled` / `readOnly`，需要禁用或只读时由宿主显式传给对应控件。它不规定 schema 格式，也不拥有远程提交。
...
- `multipleEmit` `('label'|'index'|'both')`; `label` emits selected labels for `change` / `confirm`
