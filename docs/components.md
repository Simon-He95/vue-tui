...
命令面板组件，组合 `TDialog`、`TInput` 和列表行渲染。它接收一组 `items`，按 `label` / `detail` / `keywords` 做文本过滤，`Enter` 触发 `select`，`Esc` 触发 `close`。选择命令不会自动关闭面板；宿主应在 `@select` 中更新 `v-model`。
...
`TAutocompleteInput` 组合 `TInput` 和受控 suggestions 列表。键入时透传内部 `TInput` 的 `input` / `change`；选择 suggestion 时 emit `update:modelValue`、`change` 和 `select`，不 emit `input`。

## TContextMenu

`TContextMenu` 是轻量菜单 overlay，基于现有 `TBox` / `TText` / `TView` 渲染。默认会在外部点击时关闭；如需保持 non-modal 行为，可设置 `closeOnOutside=false`。它不会直接操作系统 clipboard 或浏览器窗口；菜单项动作通过 `select` 交给宿主处理。
...
- `linkStyle` `(Style?)`：link text 叠加样式；OSC8 links 省略时默认 `{ underline: true }`；`linkify=true` 的自动链接会先叠加 `theme.components.TLink.style` 并遵循 `TLink.underline=false`
