...
> Experimental API：当前从 `@simon_he/vue-tui/experimental` 导出，暂不进入 root 入口。`ansi=true` 只支持 ANSI SGR styling；OSC8 hyperlink、minimap 和 arbitrary variable-height rich rows 仍不是当前能力。
