...
>
> Markdown link 会写入 `Style.href` metadata。DOM renderer 默认不把 `Style.href` 渲染为原生 `<a>`。如需启用链接，请显式传 `<TerminalProvider :dom-renderer-options="{ links: { activation: 'event', onActivate } }" />`。启用后，`http:`、`https:`、`mailto:` 会渲染为 `<a>`；相对链接仍保留为普通文本，除非宿主信任内容并传 `{ allowRelative: true }`。CLI/stdout renderer 会在支持时发出 OSC8 hyperlink。
>
