...
`TLogView` 会按行缓存 fixed one-line 的最终 render string。`createAppendOnlyLogStore()` 会为 completed lines 提供稳定 key，并在 tail 文本变化时更换 tail key，让 streaming / append-only 场景复用已完成历史行的 clipped/padded 输出。
...
当前 `TLogView` 只支持 fixed one-line rows。超出宽度的行会被 clip，不会 wrap；缓存也不包含 ANSI parse、syntax highlight 或 variable-height rows。
