...
Advanced 表单上下文组件，从 `@simon_he/vue-tui/vue` 引入。它提供 field registry 的最小基础：`model`、同步 `rules`、`submit`、`validation`、disabled/readOnly context，以及 `TFormField name` 对错误文本的读取。它不规定 schema 格式，也不拥有远程提交。
...
Advanced pane group，从 `@simon_he/vue-tui/vue` 引入。`sizes` 受控，separator 支持键盘微调；default slot 接收 `{ panes }`，pane 内容由宿主按返回 rect 渲染。
