...
`href` 表示 hyperlink 目标。stdout renderer 会在通过 `sanitizeTerminalHref()` 后输出 OSC8 链接；DOM renderer 会把 safe href 渲染为带 `noopener noreferrer` 的 `<a>`，unsafe href 仍按普通文本渲染。
