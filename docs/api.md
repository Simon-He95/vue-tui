...
颜色使用 `AnsiColorName`（16 色，如 `whiteBright` / `blue`）。
...
- `createDefaultTInputHostAdapter()` 提供默认的 Node-like clipboard/path 行为
...
开启 `DIMCODE_PROFILE_TUI=1` 后，profiler 会输出：
...
- 环境覆盖：`DIMCODE_COLOR_MODE` / `VUE_TUI_COLOR_MODE`
