...
把宿主能力打包成 `TInput` 插件，交给 `inputPlugins` 或局部 `plugins` 注入。`TerminalProvider.inputPlugins` 和 `TInput.plugins` 都是 init-only；运行时切换插件组合需要 remount 对应 provider/input。
...
- `@simon_he/vue-tui` 导出 browser-safe 的 `createTInputHostPlugin()`
- `createDefaultTInputHostAdapter()` 和 Node-aware 的 `defaultTInputHostPlugin` 从 `@simon_he/vue-tui/cli` 导出，提供默认的 Node-like clipboard/path 行为
...
- `<TFlex><TFlexItem /></TFlex>`：Flex 风格 row/column 布局（基于 grow/shrink/min/max/百分比尺寸/padding/margin/order/wrap/alignContent/measure 生成子 view）
...
- `<TSelect />` / `<TList />`：
  ...
  - `<TSelect multiple />`：上下键移动光标，Space 切换勾选，Enter confirm；`v-model` 由 `valueMode='index|value|option'` 决定；`multipleEmit='label|value|index|both'` 控制 `@change/@confirm` 参数（默认 `label`）
