...
- `TLogView wrap=true` 会按 visual row 滚动，并按 `getLineKey(index) + width` 缓存 plain-text wrap 结果；大日志的优化路径是 append-only bottom streaming，不支持 ANSI/highlight/rich span wrap。
