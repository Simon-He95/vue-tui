export type AgentEvent =
  ...
  | { type: "tool-log"; text: string; richLogIndex?: number }
...
export type AgentFixtureExpansion = Readonly<{
  thinkingExpanded: boolean;
  toolCallExpanded: boolean;
}>;

const defaultFixtureExpansion: AgentFixtureExpansion = Object.freeze({
...
function changedFilesBox(): string[] {
  const width = 44;
  const row = (text: string) => `  │${text.padEnd(width)}│`;
  return [
    `  ╭${"─".repeat(width)}╮`,
    row(" Changed 3 files                 +148  -23 "),
    row(" src/mock-agent-stream.ts          +36   -2 "),
    `  ╰${"─".repeat(width)}╯`,
  ];
}

export function renderBestAgentRichLog(
  index: number,
  expansion: AgentFixtureExpansion = defaultFixtureExpansion,
): string {
  const href = `https://example.com/agent-console/rich/${index}`;
  const thinking = expansion.thinkingExpanded
    ? [
    ...
    : ["\x1b[35;1mThinking ▸\x1b[0m"];
  const toolCall = expansion.toolCallExpanded
    ? [
    ...
    : ["\x1b[33;2m▸ ● Run 3 commands in:/out:/code-bg\x1b[0m"];
  return [
    ...thinking,
    "",
    ...toolCall,
...
export function createSyntheticAgentEvent(index: number): AgentEvent {
  ...
  if (index % 37 === 7) {
    return { type: "tool-log", text: renderBestAgentRichLog(index), richLogIndex: index };
  }
  if (index % 5 === 0) {
