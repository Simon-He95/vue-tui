...
- Added optional `TVideo` transport controls with click-to-pause video surfaces, drag-to-seek, 1×/2×/3× playback, and compact or cinema control layouts while keeping decoder restarts and frame conversion bounded.
...
- `stream-markdown-parser` is pinned to `1.0.0` for release candidate reproducibility.
